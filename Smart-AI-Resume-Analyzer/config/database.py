...
def init_database():
    ...
    # Create resume_radar_analysis table for Resume Radar feature
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS resume_radar_analysis (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        resume_id INTEGER,
...
def save_resume_radar_analysis(resume_id, analysis_results):
    """Save Resume Radar analysis results to database"""
    conn = get_database_connection()
    cursor = conn.cursor()
    
    try:
        global_reflection = analysis_results.get('global_reflection', {})
        
        cursor.execute('''
        INSERT INTO resume_radar_analysis (
            resume_id, global_rating, global_strengths,
            global_weaknesses, global_feedback, section_feedback,
            granular_feedback, total_feedback_items, analysis_timestamp
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            resume_id,
            global_reflection.get('rating', 0),
            str(global_reflection.get('strengths', [])),
            str(global_reflection.get('weaknesses', [])),
            global_reflection.get('feedback', ''),
            str(analysis_results.get('section_feedback', [])),
            str(analysis_results.get('granular_feedback', [])),
            analysis_results.get('total_feedback_items', 0),
            analysis_results.get('analysis_timestamp', datetime.now().isoformat())
        ))
        
        conn.commit()
        return cursor.lastrowid
    except Exception as e:
        print(f"Error saving Resume Radar analysis: {str(e)}")
        conn.rollback()
        return None
    finally:
        conn.close()

def get_resume_radar_stats():
    """Get Resume Radar analysis statistics"""
    conn = get_database_connection()
    cursor = conn.cursor()
    
    try:
        # Get total Resume Radar analyses
        cursor.execute('SELECT COUNT(*) FROM resume_radar_analysis')
        total_analyses = cursor.fetchone()[0]
        
        # Get average global rating
        cursor.execute('SELECT AVG(global_rating) FROM resume_radar_analysis WHERE global_rating > 0')
        ...
        SELECT 
            CASE 
                WHEN global_rating >= 17 THEN 'Excellent'
                WHEN global_rating >= 13 THEN 'Good'
                WHEN global_rating >= 9 THEN 'Average'
                WHEN global_rating >= 5 THEN 'Poor'
                ELSE 'Very Poor'
            END as rating_category,
            COUNT(*) as count
        FROM resume_radar_analysis 
        ...
        # Get recent analyses
        cursor.execute('''
        SELECT rr.global_rating, rd.name, rr.created_at
        FROM resume_radar_analysis rr
        JOIN resume_data rd ON rr.resume_id = rd.id
        ...
        print(f"Error getting Resume Radar stats: {str(e)}")
        return {
            'total_analyses': 0,
            'average_rating': 0,
            'rating_distribution': {},
